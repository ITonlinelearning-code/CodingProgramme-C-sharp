    ...
    public class PersonManager
    {
        ...
        public string YearsHistory()
        {
            ...
            try
            {
                ...
                lastMessage += $"{Environment.NewLine}You were born in the year: {year}{Environment.NewLine}Here are the years of your life so far:";
                Console.WriteLine();

                if (_person.Age < 17)
                {
                    lastMessage += "You are too young to own a car.";
                }
                else
                {
                    lastMessage += "You are of legal driving age.";
                }

                for (int i = 0; i < _person.Age; i++)
                {
                    year++;
                    lastMessage += $"In {year}, you turned {i + 1}.";

                    switch (i)
                    {
                        case < 10:
                            lastMessage += "You were a child.";
                            break;
                        case < 20:
                            lastMessage += "You were a teenager.";
                            break;
                        case < 60:
                            lastMessage += "You were an adult.";
                            break;
                        case >= 60:
                            lastMessage += "You are elderly.";
                            break;
                    }
                }
            }
